...
async def authenticate():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            return data.get("access_token"), email  # Возвращаем токен и email
...
async def send_task(token: str, user_id: str, action: str, data: dict):
    ...
    async with websockets.connect(uri, extra_headers=headers) as websocket:
        ...
        # Начать прослушивание в отдельной задаче
...
async def main():
    ...
    token, email = await authenticate()  # Получаем токен и email
    user_id = input("Enter user ID (should match your email): ")  # Уточняем, что user_id должен совпадать с email
    if user_id != email:
